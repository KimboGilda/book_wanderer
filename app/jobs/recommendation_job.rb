class RecommendationJob < ApplicationJob
  ...
  def perform(user_id)
      ...
      @read = ReadBook.where(user_id: user_id)
...
# work till here
  ...
  # puts @books_for_carousel
  # p "HERE"


    # Return the array of book objects

    # @books_for_carousel.each do |book|
    #   Recommendation.find_or_create_by(user: user, book: book)
    # end
    # recomend = Recommendation.all
    # p recomend
  end
